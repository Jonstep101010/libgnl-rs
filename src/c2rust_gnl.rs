...
pub unsafe extern "C" fn shift_static_buffer(mut static_buffer: *mut libc::c_char) {
    let mut newline_pos: *const libc::c_char = strchr(
        static_buffer as *const libc::c_char,
        '\n' as i32,
    );
    ...
    let shift_len: size_t = ((20 as libc::c_int + 1 as libc::c_int) as size_t)
        .wrapping_sub(start);
    memmove(
        static_buffer as *mut libc::c_void,
...
pub unsafe extern "C" fn read_newln(
    ...
    if *temp_buffer.as_mut_ptr() != 0 {
        *count -= 20 as libc::c_int;
        let mut newline: *const libc::c_char = strchr(
            temp_buffer.as_mut_ptr(),
            '\n' as i32,
        );
        if !newline.is_null() {
            let len: libc::c_int = (if (newline.offset_from(temp_buffer.as_mut_ptr())
                as libc::c_long) < 20 as libc::c_int as libc::c_long
            {
...
pub unsafe extern "C" fn get_next_line(mut fd: libc::c_int) -> *mut libc::c_char {
    if (20 as libc::c_int) < 1 as libc::c_int || fd > 10240 as libc::c_int
        || fd < 0 as libc::c_int
    {
        return 0 as *mut libc::c_char;
    }
    ...
    if count <= 20 as libc::c_int
        && static_buffer[fd as usize][count as usize] as libc::c_int == '\n' as i32
    {
        return terminated_line_copy(
            read_buffer((static_buffer[fd as usize]).as_mut_ptr()),
        );
    }
    return terminated_line_copy(
        read_newln(
